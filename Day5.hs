...
program1:: [(Int, Int)] -> [[Int]] -> Int
program1 [] _ = 0
program1 _ [] = 0
program1 x (y:ys)
    | rulesPass x (Map.fromList (zip y [0..])) = middle y + program1 x ys
    | otherwise = program1 x ys

rulesPass :: [(Int, Int)] -> Map.Map Int Int -> Bool
rulesPass [] _ = True
rulesPass ((x, y):zs) q = 
    case (findIndexInMap x q, findIndexInMap y q) of
        (Just indexX, Just indexY) -> indexX < indexY && rulesPass zs q
        _ -> rulesPass zs q

findIndexInMap :: Int -> Map.Map Int Int -> Maybe Int
findIndexInMap = Map.lookup

-- inspired by https://stackoverflow.com/a/1735541
middle :: [a] -> a
...
program2:: [(Int, Int)] -> [[Int]] -> Int
program2 [] _ = 0
program2 _ [] = 0
program2 x (y:ys)
    | rulesPass x (Map.fromList (zip y [0..])) = program2 x ys
    | otherwise = middle (Map.keys (ruleSort x (Map.fromList (zip y [0..])))) + program2 x ys

ruleSort :: [(Int, Int)] -> Map.Map Int Int -> Map.Map Int Int
ruleSort [] m = m
ruleSort ((x,y):z) m = 
    case (findIndexInMap x m, findIndexInMap y m) of
        (Just indexX, Just indexY) -> 
            if indexX > indexY 
            then trace (show x) trace (show y) ruleSort z (cleanIndexes (Map.insert y indexX (Map.insert x indexY m)))
            else ruleSort z m
        _ -> ruleSort z m

updateKeyForValue :: Int -> Int -> Map.Map Int Int -> Map.Map Int Int
updateKeyForValue value newKey m =
    let maybeKey = fmap fst . Map.lookupMin $ Map.filter (== value) m
    in case maybeKey of
        Just oldKey -> Map.insert newKey value (Map.delete oldKey m)
        Nothing -> m 

cleanIndexes :: Map.Map Int Int -> Map.Map Int Int
cleanIndexes m = Map.fromList (zip (Map.keys m) [0..])
...
main =  do
    print "Result 1:"
    print $ program1 testX testY
    print "Result 2:"
    print $ program2 testX testY
